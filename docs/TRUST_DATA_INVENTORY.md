...
**Evidence snapshot:** core repository `f75493fcb2639f01c69279048302144bbd9181f3`  
**Reviewed:** 2026-08-28  
...
| V     | Onboarding and entitlement metadata                                | Record completion/version and application access state                                              | Supabase user-scoped tables and authenticated endpoint                                                                                                        | RLS/grants; Supabase/Vercel                                                                                                                                                     | Cascades with auth user where defined; no separate time limit in migrations                                                                              | Complete onboarding; account deletion                                                           | `api/onboarding.ts`; `supabase/migrations/20260821125500_add_onboarding_and_stripe_billing.sql`; `supabase/migrations/20260820120000_add_user_entitlements.sql`                         | Confirm obsolete Stripe tables contain no production records and approve a separate removal/migration; no Stripe runtime integration was found               |
...
No active advertising service, Stripe runtime, Sentry, PostHog, or direct OpenAI
...
deployed AI repository contain no additional integration. Historical Stripe-named
database tables remain in migrations and require the explicit follow-up recorded
below; they must not be silently represented as an active payment processor.
...
| HC-09 | Confirm whether legacy Stripe-named tables ever received production data, then separately approve retention/export/removal; do not list Stripe as active without evidence                                     | Owner + database operator       | Accurate historical processor/data register |
