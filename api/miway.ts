...
type TripDescriptor = { tripId: string | undefined; routeId: string | undefined; directionId: number | undefined };
type StopPrediction = { stopId: string | undefined; arrival: number | undefined; departure: number | undefined };
type TripUpdate = { trip: TripDescriptor; timestamp: number | undefined; stops: StopPrediction[] };
...
const EMPTY_TRIP: TripDescriptor = { tripId: undefined, routeId: undefined, directionId: undefined };
...
function fields(bytes: Uint8Array): WireField[] {
  ...
  while (offset < bytes.length) {
    ...
    if (wire === 2) {
      ...
      if (!Number.isSafeInteger(size) || size < 0 || offset + size > bytes.length) throw new Error("Invalid protobuf length.");
...
function tripDescriptor(bytes: Uint8Array): TripDescriptor {
  ...
  return { tripId: stringField(list, 1), routeId: stringField(list, 5), directionId: numberField(list, 6) };
}
...
function tripUpdate(bytes: Uint8Array): TripUpdate {
  ...
  const stops = list.filter((field) => field.field === 2).map(nested).filter((value): value is Uint8Array => value !== null).map((value): StopPrediction => {
  ...
  return { trip: tripBytes ? tripDescriptor(tripBytes) : EMPTY_TRIP, stops, timestamp: numberField(list, 4) };
}
...
function vehiclePosition(bytes: Uint8Array): VehiclePosition {
  ...
  const vehicle: VehicleDescriptor = vehicleBytes ? vehicleDescriptor(vehicleBytes) : { id: undefined, label: undefined };
  return {
...
function feedEntities(bytes: Uint8Array): { timestamp: number | undefined; entities: WireField[][] } {
  const list = fields(bytes);
  const headerBytes = nestedField(list, 1);
  return {
    timestamp: headerBytes ? numberField(fields(headerBytes), 3) : undefined,
    entities: list.filter((field) => field.field === 2).map(nested).filter((value): value is Uint8Array => value !== null).map(fields),
  };
}
function parseVehicleFeed(bytes: Uint8Array): ParsedFeed<VehiclePosition> {
  const feed = feedEntities(bytes);
  return { timestamp: feed.timestamp, items: feed.entities.map((entity) => nestedField(entity, 4)).filter((value): value is Uint8Array => value !== null).map(vehiclePosition) };
}
function parseTripFeed(bytes: Uint8Array): ParsedFeed<TripUpdate> {
  const feed = feedEntities(bytes);
  return { timestamp: feed.timestamp, items: feed.entities.map((entity) => nestedField(entity, 3)).filter((value): value is Uint8Array => value !== null).map(tripUpdate) };
}
...
async function fetchBytes(url: string, signal: AbortSignal): Promise<Uint8Array> {
  const response = await fetch(url, { signal, headers: { Accept: "application/x-protobuf, application/protobuf, application/octet-stream", "User-Agent": "Gapwise/1.0 (+https://gapwise.ca)" } });
...
async function buildSnapshot(): Promise<Snapshot> {
  ...
  try {
    const [vehicleBytes, tripBytes] = await Promise.all([fetchBytes(VEHICLE_POSITIONS_URL, controller.signal), fetchBytes(TRIP_UPDATES_URL, controller.signal)]);
    const vehicleFeed = parseVehicleFeed(vehicleBytes);
    const tripFeed = parseTripFeed(tripBytes);
    const updatesByTrip = new Map(tripFeed.items.filter((update) => update.trip.tripId).map((update) => [update.trip.tripId!, update] as const));
    ...
    const vehicles = vehicleFeed.items
      .map((vehicle) => ({ vehicle, update: vehicle.trip.tripId ? updatesByTrip.get(vehicle.trip.tripId) : undefined }))
      .map(({ vehicle, update }) => ({ vehicle, update, routeId: vehicle.trip.routeId ?? update?.trip.routeId }))
      .filter(({ vehicle, routeId }) => Boolean(routeId && UTM_ROUTE_IDS.has(routeId) && Number.isFinite(vehicle.lat) && Number.isFinite(vehicle.lng) && distanceKm(vehicle.lat!, vehicle.lng!, UTM_CENTER.lat, UTM_CENTER.lng) <= MAX_DISTANCE_FROM_UTM_KM))
      .map(({ vehicle, update, routeId }) => {
        const utmStop = update?.stops.find((stop) => stop.stopId && UTM_STOP_IDS.has(stop.stopId) && (stop.arrival ?? stop.departure ?? 0) >= nowSeconds - 90);
        const etaAt = utmStop?.arrival ?? utmStop?.departure;
        const eta = etaAt && etaAt >= nowSeconds - 90 ? Math.max(0, etaAt - nowSeconds) : null;
        const atUtm = (vehicle.stopId ? UTM_STOP_IDS.has(vehicle.stopId) : false) || distanceKm(vehicle.lat!, vehicle.lng!, UTM_CENTER.lat, UTM_CENTER.lng) < 0.22;
        return {
          id: vehicle.vehicleId ?? `${vehicle.trip.tripId ?? routeId}-${vehicle.label ?? "vehicle"}`,
          route: routeId!, tripId: vehicle.trip.tripId ?? null, label: vehicle.label ?? null,
...
async function snapshot(): Promise<Snapshot> {
  ...
  const pending = buildSnapshot().catch((error) => { cachedSnapshot = null; throw error; });
  cachedSnapshot = { expiresAt: now + SHARED_CACHE_MS, pending };
  return pending;
}
export default async function handler(request: Request): Promise<Response> {
  if (request.method !== "GET") return json({ error: "Method not allowed" }, 405);
  try { return json(await snapshot()); }
  catch {
    return json({ status: "unavailable", generatedAt: new Date().toISOString(), sourceObservedAt: null, vehicles: [] } satisfies Snapshot, 503);
  }
}
