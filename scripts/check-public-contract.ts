...
for (const [path, text] of docs) {
  ...
  if (
    /(?:npm install @gapwise\/sdk|pip install gapwise|pip3 install gapwise)/.test(text) &&
    !/not published|not yet published|after (?:the )?registry release/i.test(text)
  )
    fail(
      `${path} implies registry installation is currently available; keep the human release gate explicit.`,
    );
}
