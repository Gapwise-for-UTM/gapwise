...
const SENSITIVE_ASSIGNMENT =
  /\b(authorization|access[_-]?token|refresh[_-]?token|id[_-]?token|client[_-]?secret|api[_-]?key|apikey|service[_-]?role(?:[_-]?key)?|password|secret|private[_-]?key|kek)\b(\s*[:=]\s*)(["']?)([^\s,"';&}]+)\3/giu;
const SUPABASE_AUTH_STORAGE = /(sb-[A-Za-z0-9_-]+-auth-token)(\s*[:=]\s*)(["']?)([^\s,"';&}]+)\3/giu;
...
export function redactSensitiveLogText(value: string): string {
  return value
    ...
    .replace(SUPABASE_AUTH_STORAGE, (_match, key: string, separator: string) => {
      return `${key}${separator}${REDACTED}`;
    });
}
